...
R_InitGraphics( int width, int height )
{
	...
	d_pzbuffer = (zvalue_t*)malloc(width * height * sizeof(zvalue_t));
...
{
	swap_current = 0;
	swap_buffers = (pixel_t*)malloc(height * width * sizeof(pixel_t) * 2);
	if (!swap_buffers)
	{
	...
	sintable = (int*)malloc((width+CYCLE) * sizeof(int));
	intsintable = (int*)malloc((width+CYCLE) * sizeof(int));
	blanktable = (int*)malloc((width+CYCLE) * sizeof(int));

	newedges = (edge_t**)malloc(width * sizeof(edge_t *));
	removeedges = (edge_t**)malloc(width * sizeof(edge_t *));

	warp_rowptr = (byte**)malloc((width+AMP2*2) * sizeof(byte*));
	warp_column = (int*)malloc((width+AMP2*2) * sizeof(int));
	...
	r_warpbuffer = (pixel_t*)malloc(height * width * sizeof(pixel_t));
	...
	vid_polygon_spans = (espan_t*)malloc(sizeof(espan_t) * (height + 1));
...
{
	int x, y;
	byte *buffer = (byte*)malloc(vid_buffer_width * vid_buffer_height * 3);
